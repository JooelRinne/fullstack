...
describe('favourite blog', () => {
  ...
  test('when list has many blogs favourite blog is the one with most likes', () => {
    const result = listHelper.favouriteBlog(listWithAlotOfBlogs)
    expect(result).toEqual({ title: listWithAlotOfBlogs[2].title, author: listWithAlotOfBlogs[2].author, likes: listWithAlotOfBlogs[2].likes })
  })

  test('when list is empty equals undefined', () => {
    const result = listHelper.favouriteBlog(listWithNoBlogs)
    expect(result).toEqual({ title: undefined, author: undefined, likes: undefined })
  })
})
...
describe('most blogs', () => {
  ...
  test('when list has many blogs the author with most blogs is returned', () => {
    const result = listHelper.mostBlogs(listWithAlotOfBlogs)
    expect(result).toEqual({ author: 'Robert C. Martin', blogs: 3 })
  })

  test('when list is empty the author with most blogs is undefined and amount of blogs zero', () => {
